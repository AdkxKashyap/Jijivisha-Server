...
class IsDataUpdated(Resource):
    def get(self):
        try:

            date = request.args.get('date')
            date = date[: date.rfind(':')]
            my_date = datetime.strptime(date, '%Y-%m-%d %H:%M')
            is_updated = True
            # print(date)
            latest_pub_date = CheckLatestData().getData()
            if(latest_pub_date > my_date):
                is_updated = False
            # for data in res:
            #     list_.append(data)
            return Response(dumps(is_updated), status=200, mimetype='application/json')
        except Exception as err:
            print(err)
            return Response(dumps({"error": str(err)}), status=500, mimetype='application/json')
...
api.add_resource(GetLatestIndiaDistrictData,
                 '/Jijivisha/v1.0/GetLatestIndiaDistrictData')
...
api.add_resource(IsDataUpdated, '/Jijivisha/v1.0/IsDataUpdated')
# driver function
